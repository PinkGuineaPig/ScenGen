# Backend/app/routes/som_configs.py
...
def list_configs(run_id):
    ...
    # Fetch SOM configs by the run's config_id
    configs = (SOMProjectionConfig.query
...
def get_config(run_id, cfg_id):
    # Ensure run exists and fetch config by config_id
    run = ModelRun.query.filter_by(id=run_id).first_or_404(description="Run not found")
    c = (SOMProjectionConfig.query
